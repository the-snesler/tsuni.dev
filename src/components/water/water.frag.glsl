...
void main() {
    // Convert UV coordinates to screen coordinates for the water function
    vec2 fragCoord = vUv * uResolution;
    
    // Use camera direction from uniform
    vec3 cameraDir = normalize(uCameraDirection);
    
    // Generate water color
    vec3 waterColor = water(vec2(fragCoord.x / 32.0 - uTime / 4.0, fragCoord.y / 16.0  - uTime / 6.0), cameraDir);
